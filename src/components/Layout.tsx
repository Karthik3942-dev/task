...
function Layout() {
  ...
  useEffect(() => {
    const checkUserRole = async () => {
      ...
      try {
        // Check if user is in teams as a creator (team leader/manager)
        const teamsQuery = query(collection(db, "teams"), where("created_by", "==", user.uid));
        const teamsSnapshot = await getDocs(teamsQuery);

        // Check if user has management permissions or is a team creator
        if (!teamsSnapshot.empty) {
          setUserRole("manager");
          setIsManager(true);
        } else {
          // Check if user exists in employees collection and has specific role
          const employeesQuery = query(collection(db, "employees"), where("uid", "==", user.uid));
          ...
          if (!employeesSnapshot.empty) {
            ...
            setUserRole(role);
            setIsManager(role === "manager" || role === "admin" || role === "team_lead");
          } else {
            // Default: assume manager if not specifically in employees collection
            setUserRole("manager");
            setIsManager(true);
          }
        }
  ...
  return (
    <div className="min-h-screen h-screen bg-gray-50 dark:bg-black flex overflow-hidden">
      ...
      <div
        ...
        } md:translate-x-0 fixed md:static inset-y-0 left-0 z-50 w-[200px] bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 transition-all duration-300 ease-in-out flex flex-col`}
      >
        {/* Brand Header */}
        ...
        <div className="flex-1 overflow-y-auto px-2 py-2">
          <nav className="space-y-1">
            ...
            {/* Team Management Section - Only show for managers */}
            {isManager && (
              <div>
                <button
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-4 py-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium text-gray-900 dark:text-white">Project Board</span>
            ...
            <div className="flex items-center gap-2">
              {isManager && (
                <Link
                  to="/TeamManager"
                  className="px-2 py-1 text-xs bg-cyan-500 text-white rounded-md hover:bg-cyan-600 transition-colors"
                >
                  Teams
                </Link>
              )}
              ...
              <button
                onClick={toggleTheme}
                className="p-1.5 rounded-md bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors"
        ...
        {/* Page Content with Wider Layout */}
        <main className="flex-1 flex flex-col min-h-0 bg-gray-50 dark:bg-black">
